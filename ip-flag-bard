...
const concurrency = new Concurrency(10);
const cache = new Cache();

async function operator(proxies) {
  const tasks = proxies.map(async proxy => {
    const countryCode = await cache.get(proxy) ?? await queryIpApi(proxy);
    cache.set(proxy, countryCode);

    proxy.name = `${removeFlag(proxy.name)} ${getFlagEmoji(countryCode)}`;
  });

  await concurrency.all(tasks);
